...
- `nanosecond-timestamps` -- nanosecond timestamp primitive types (experimental). Gates
  support for `timestamp_nanos` and `timestamp_nanos_ntz` data types, primitive parsing/conversion,
  and table operations involving nanosecond precision timestamps. Auto-enables the feature in
  dependent crates (`default_engine`, `ffi`, `test_utils`).
...
- Always cross-check protocol behavior against the
  [Delta protocol spec](https://raw.githubusercontent.com/delta-io/delta/kernel/PROTOCOL.md)
